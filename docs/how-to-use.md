...
Docker の `4000:4000` は、通常すべてのホスト側ネットワークインターフェースにポートを公開します。既定構成は `127.0.0.1:4000:4000` とし、Mac自身からだけ利用します。非TLSのままLANへ公開するとログイン情報と財務データが平文で流れるため、別端末から使う場合はVPNまたはHTTPSを終端するリバースプロキシを使用してください。
...
平文HTTPのLAN公開は行いません。VPNでMacのlocalhostへ安全に接続するか、Caddy/Nginx等でHTTPSを終端し、`TRUSTED_PROXIES`をプロキシのIP/CIDRだけに限定して `FORCE_HTTPS=true` を設定してください。

移行中の閉域網でリスクを受容する場合に限り、`WEB_EXTERNAL_HOST=<MacのLAN IP>` と `ALLOW_INSECURE_HTTP=true` を明示すると起動できます。この設定では認証情報と財務データが暗号化されないため、恒久運用には使用しないでください。
...
services:
  omni-money:
    ...
    environment:
      ...
      HOST_IP: 0.0.0.0
      PORT: "4000"
      WEB_EXTERNAL_HOST: 0.0.0.0
      # 閉域LANでの移行用途だけ。恒久運用はHTTPSプロキシへ置き換える。
...
ログイン画面で、bcrypt ハッシュ作成時に入力した元のパスワードを入力します。この例は `ALLOW_INSECURE_HTTP=true` を明示した閉域LAN向けの移行構成で、認証情報と財務データは暗号化されません。恒久運用ではHTTPSを終端するリバースプロキシへ移行し、この許可を削除してください。
...
| 別の Mac から接続できない | `127.0.0.1:4000:4000` は安全のためローカル限定です。LAN利用はVPNまたはHTTPSリバースプロキシを構成します。 |
